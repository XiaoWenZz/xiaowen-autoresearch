...
class ControllerHeartbeatAutomationTest(unittest.TestCase):
    def test_frozen_singleton_fixture_uses_native_controller_recovery_contract(self) -> None:
        ...
        for phrase in (
            ...
            "exact remote-job projection or explicit no-remote-job",
            "source_turn_state=IN_PROGRESS|FINAL",
            "FINAL+NON_TERMINAL",
            "live interactive authentication/credential/approval UI or PTY",
            "commentary-only IN_PROGRESS",
            "preserve the cursor and PTY",
            "same-owner terminal-recovery wake",
            "prebound ACTIVE/NONE job is not submission evidence",
            "absent unit plus absent files before late_threshold has zero effect",
