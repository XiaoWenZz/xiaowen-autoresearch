...
Internal portfolio sequencing is never a blocker authority. “Wait for another
candidate,” throughput recovery, context rollover, owner attention, governance
cleanup, or a preferred ordering among runnable ideas does not satisfy
`EXTERNAL_FACT` or `UNAVAILABLE_AUTHORITY`. Keep the cheapest action owned, or
scientifically close the exact scope; do not park it as `BLOCKED` to reduce the
visible active set.
...
At the start of a singleton wake, before loading the full Skill, canonical
snapshot or long references, run the read-only
`workflow_evolution_gate.py controller-context-window`, then read
`controller_control_state.py show --projection active`. If that projection and
the incoming event contain no active objective, role, job, advisory, pending
absorption or callback obligation, return quietly without paying the full
reload cost. An actionable wake then loads the complete Skill plus only its
triggered reference files before effects. Use `show --projection full` only
for migration, contradiction, rebuild or replay; every mutation command still
reads and validates the full canonical state.

The context gate resets after the latest top-level `compacted` or
`event_msg/context_compacted`, reads at most the latest 20
`last_token_usage.input_tokens`, and returns exactly `ALLOW`,
`REQUIRE_ROLLOVER` when at least 8 samples have median above 96000, or
`PAUSE_NEW_OBJECTIVE_ADMISSION` after 20 consecutive values above 128000.
p95 remains diagnostic.
...
- Resolve the configured project and conversation by exact runtime metadata ID
  ...
  conversation.
- Run the cheapest decisive local source/code/algebra/replay check before Pro.
  The first batch in a conversation/account, any applicable authority-hash
  change, uncertain retained context, or prospectively fresh independent review
  submits one exact-hashed full authority bundle: the complete current Skill
  directory, every applicable parent-to-child `AGENTS.md`, routed direct
  references, candidate diff/validation and bounded decision evidence. A later
  batch in the exact same conversation may use a delta only when the prior full
  bundle hash and every applicable authority hash are unchanged: send that
  bundle hash, the unchanged manifest, changed files/diff, validation and
  evidence delta. Any mismatch or retention uncertainty falls back to full.
  Package only Git-tracked Skill files plus explicitly admitted external
  authorities; exclude `.git`, caches, generated bytecode and unrelated
  histories. Exclude only secrets, credentials and protected model/data/result
  payloads. Name every exclusion and hash the files actually sent so later Pro
  review is reproducible; reuse the current task record rather than creating a
  packet registry or artifact family.
